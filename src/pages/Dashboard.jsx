  ...
  return (
    <>
      <style>{`
        ...
        #db-root { position:fixed; inset:0; display:flex; background:#F1F4F8; font-family:Inter,sans-serif; z-index:100; overflow:hidden; }
        ...
        .db-nav-link { display:flex; align-items:center; gap:9px; padding:8px 10px; border-radius:7px; font-size:13px; color:rgba(255,255,255,.65); cursor:pointer; margin-bottom:1px; text-decoration:none; transition:background .15s, color .15s; }
        .db-nav-link:hover { background:rgba(255,255,255,.07); color:#fff; }
        .db-nav-link.active { background:#4A90C4; color:#fff; font-weight:500; }
        .db-stat { background:#fff; border:1px solid #DDE4EE; border-radius:12px; padding:18px 20px; cursor:pointer; transition:box-shadow .15s,transform .15s; position:relative; overflow:hidden; text-decoration:none; display:block; }
        .db-stat:hover { box-shadow:0 4px 18px rgba(30,58,95,.08); transform:translateY(-1px); }
        .db-stat::after { content:''; position:absolute; top:0; left:0; right:0; height:3px; border-radius:12px 12px 0 0; background:#4A90C4; opacity:0; transition:opacity .15s; }
        .db-stat:hover::after { opacity:1; }
        .db-stat.warn::after { background:#C05621; opacity:1; }
        .db-quick { background:#fff; border:1px solid #DDE4EE; border-radius:12px; padding:16px; cursor:pointer; transition:all .15s; display:flex; flex-direction:column; align-items:flex-start; gap:8px; text-decoration:none; }
        .db-quick:hover { border-color:#4A90C4; box-shadow:0 3px 14px rgba(74,144,196,.12); }
        .db-act-row { width:100%; display:flex; align-items:flex-start; gap:12px; padding:12px 20px; border:none; background:transparent; cursor:pointer; text-align:left; transition:background .12s; }
        .db-act-row:hover { background:#FAFCFF; }
        .db-al-row { width:100%; display:flex; align-items:center; gap:12px; padding:11px 20px; border:none; background:transparent; cursor:pointer; text-align:left; transition:background .12s; }
        .db-al-row:hover { background:#FFFAF6; }
        .db-ec-row { width:100%; display:flex; align-items:center; gap:10px; padding:10px; border-radius:9px; border:none; background:transparent; cursor:pointer; text-align:left; transition:background .12s; }
        .db-ec-row:hover { background:#F1F4F8; }
        .db-g4 { display:grid; grid-template-columns:repeat(4,1fr); gap:14px; margin-bottom:24px; }
        .db-2col { display:grid; grid-template-columns:1fr 340px; gap:18px; margin-bottom:18px; }
        @media(max-width:1100px){ .db-g4{grid-template-columns:repeat(2,1fr);} .db-2col{grid-template-columns:1fr;} }
      ...
      <div id="db-root">
        ...
        <div id="db-sidebar">
          <div style={{ padding:'22px 18px 18px', borderBottom:'1px solid rgba(255,255,255,.08)' }}>
            <p style={{ fontFamily:'DM Serif Display,serif', fontSize:16, color:'#fff', lineHeight:1.2 }}>Arbre Décisionnel EDA</p>
            <p style={{ fontSize:10, fontWeight:500, letterSpacing:'0.1em', textTransform:'uppercase', color:'rgba(255,255,255,.4)', marginTop:3 }}>Psy-EN · RASED</p>
          </div>
          <nav style={{ flex:1, padding:'14px 10px', overflowY:'auto' }}>
            {['Principal','Gestion','Rapports'].map(g => (
              <div key={g}>
                <p style={{ fontSize:9.5, textTransform:'uppercase', letterSpacing:'0.1em', color:'rgba(255,255,255,.3)', padding:'14px 8px 5px', fontWeight:500 }}>{g}</p>
                {NAV.filter(n => n.g === g).map(n => (
                  <Link key={n.to} to={n.to} className={`db-nav-link${n.active ? ' active' : ''}`}>
          ...
          <div style={{ padding:'14px 16px', borderTop:'1px solid rgba(255,255,255,.08)', display:'flex', alignItems:'center', gap:10 }}>
            ...
            <div style={{ minWidth:0 }}>
              ...
              <p style={{ fontSize:10.5, color:'rgba(255,255,255,.4)' }}>{membres.find(m => m.email === user?.email)?.profession || 'RASED'}</p>
            </div>
          </div>
        </div>
        ...
        <div id="db-main">
          {/* Topbar */}
          <div style={{ background:'#fff', borderBottom:'1px solid #DDE4EE', height:56, padding:'0 28px', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
            <div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <div style={{ position:'relative', width:34, height:34, borderRadius:8, background:'#F1F4F8', border:'1px solid #DDE4EE', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', fontSize:16 }}>
                🔔
                <span style={{ position:'absolute', top:6, right:6, width:7, height:7, borderRadius:'50%', background:'#C05621', border:'1.5px solid #fff' }} />
              </div>
              <button onClick={() => setDiagModal(true)}
                style={{ padding:'7px 16px', borderRadius:8, fontSize:12.5, fontWeight:500, background:'#4A90C4', color:'#fff', border:'none', cursor:'pointer' }}>
                + Nouveau diagnostic
              </button>
            </div>
          </div>
          ...
          <div id="db-scroll">
            ...
            <div className="db-g4">
              {[
                ...
                <Link key={i} to={c.to} className={`db-stat${c.warn ? ' warn' : ''}`}>
                  ...
                  <p style={{ fontFamily:'DM Serif Display,serif', fontSize:32, lineHeight:1, color:c.warn && c.val > 0 ? '#C05621' : '#1A2E45', marginBottom:3 }}>{loading ? '—' : c.val}</p>
            ...
            <div className="db-g4">
              {[
                ...
                const inner = (
                  <>
                    <div style={{ width:40, height:40, borderRadius:10, background:c.bg, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20 }}>{c.ico}</div>
                ...
                return c.action
                  ? <button key={i} onClick={c.action} className="db-quick" style={{ border:'1px solid #DDE4EE' }}>{inner}</button>
                  : <Link key={i} to={c.to} className="db-quick">{inner}</Link>;
              })}
            </div>
            ...
            <div className="db-2col">
              ...
              <div style={{ background:'#fff', border:'1px solid #DDE4EE', borderRadius:12, overflow:'hidden' }}>
                <div style={{ padding:'16px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:'1px solid #DDE4EE' }}>
                ...
                {recentActivity.length === 0
                  ...
                  : recentActivity.map((e, i) => {
                    ...
                    return (
                      <button key={e.id} className="db-act-row" onClick={() => navigate(`/detail-fiche?id=${e.id}`)}
                        style={{ borderBottom: i < recentActivity.length-1 ? '1px solid #F1F4F8' : 'none' }}>
                        <div style={{ width:34, height:34, borderRadius:'50%', background: isNew ? '#E6F4ED' : '#EBF4FB', display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, flexShrink:0, marginTop:1 }}>
                          {isNew ? '👤' : '🧠'}
                        </div>
                        <div style={{ flex:1, minWidth:0 }}>
                          <p style={{ fontSize:13, fontWeight:500, color:'#1A2E45', marginBottom:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                            {isNew ? 'Fiche créée' : 'Fiche mise à jour'} — <strong style={{ color:'#2C5282' }}>{e.prenom} {e.nom}</strong>
                          </p>
                          <p style={{ fontSize:11.5, color:'#5A6E87' }}>{[e.ecole, e.classe].filter(Boolean).join(' · ') || 'Fiche élève'}</p>
                        </div>
                        <p style={{ fontSize:11, color:'#5A6E87', flexShrink:0, marginTop:3 }}>{timeAgo(e.updated_date || e.created_date)}</p>
                      </button>
                    );
                  })}
              </div>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
                ...
                <div style={{ background:'#fff', border:'1px solid #DDE4EE', borderRadius:12, overflow:'hidden' }}>
                  ...
                  {alertes5.length === 0
                    ...
                    : alertes5.map((e, i) => (
                      <button key={e.id} className="db-al-row" onClick={() => navigate(`/detail-fiche?id=${e.id}`)}
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          <p style={{ fontSize:11.5, color:'#5A6E87', marginTop:1 }}>{[e.classe, e.ecole].filter(Boolean).join(' · ') || '—'}</p>
                        </div>
                        <span style={{ fontSize:11, fontWeight:600, color:'#C05621', background:'#FEF0E6', padding:'2px 8px', borderRadius:10, flexShrink:0 }}>+{daysSince(e.updated_date || e.created_date)}j</span>
                      </button>
                    ))}
                </div>
                ...
                <div style={{ background:'#fff', border:'1px solid #DDE4EE', borderRadius:12, overflow:'hidden' }}>
                  ...
                  <div style={{ padding:'8px 12px', display:'flex', flexDirection:'column', gap:4 }}>
                    {ecolesStats.length === 0
                      ? <p style={{ textAlign:'center', padding:'20px 0', fontSize:13, color:'#5A6E87' }}><Link to="/mes-ecoles" style={{ color:'#4A90C4' }}>Ajouter une école →</Link></p>
                      : ecolesStats.map(ec => (
                        <button key={ec.id} className="db-ec-row" onClick={() => navigate(`/detail-ecole?id=${ec.id}`)}>
                          <div style={{ width:36, height:36, borderRadius:9, background:'#1E3A5F', display:'flex', alignItems:'center', justifyContent:'center', fontSize:17, flexShrink:0 }}>🏫</div>
                          <div style={{ flex:1, minWidth:0 }}>
                            <p style={{ fontSize:12.5, fontWeight:600, color:'#1A2E45', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{ec.nom}</p>
                            <p style={{ fontSize:11, color:'#5A6E87', marginTop:1 }}>{ec.type || 'École'} · {ec.nbEl} élève{ec.nbEl !== 1 ? 's' : ''}</p>
                          </div>
                          <span style={{ fontSize:10.5, fontWeight:600, padding:'2px 8px', borderRadius:10, flexShrink:0, background:ec.nbAl > 0 ? '#FEF0E6' : '#E6F4ED', color:ec.nbAl > 0 ? '#C05621' : '#276749' }}>
            ...
            {/* ÉQUIPE RASED */}
            {membres.length > 0 && (
              <div style={{ background:'#fff', border:'1px solid #DDE4EE', borderRadius:12, padding:'16px 20px', display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
                <p style={{ fontSize:12, fontWeight:600, color:'#5A6E87', textTransform:'uppercase', letterSpacing:'0.06em', flexShrink:0 }}>Équipe RASED</p>
                <div style={{ display:'flex', flexWrap:'wrap', gap:10, flex:1 }}>
                  {membres.map(m => {
                    const bg   = profColor[m.profession] || '#4A90C4';
                    const init = `${m.prenom?.[0] || ''}${m.nom?.[0] || ''}`;
                    return (
                      <div key={m.id} style={{ display:'flex', alignItems:'center', gap:7, background:'#F1F4F8', border:'1px solid #DDE4EE', borderRadius:20, padding:'4px 12px 4px 5px' }}>
                        ...
                        <div>
                          <p style={{ fontSize:12, fontWeight:500, color:'#1A2E45', lineHeight:1 }}>{m.prenom} {m.nom}</p>
                ...
                <Link to="/invite-users" style={{ padding:'7px 15px', borderRadius:8, fontSize:12.5, fontWeight:500, color:'#1A2E45', border:'1px solid #DDE4EE', textDecoration:'none', flexShrink:0, background:'#fff' }}>
                  + Inviter un membre
                </Link>
              </div>
            )}
        ...
        {diagModal && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            ...
            <motion.div initial={{ y:40, opacity:0 }} animate={{ y:0, opacity:1 }} exit={{ y:40, opacity:0 }}
              ...
              style={{ background:'#fff', borderRadius:16, width:'100%', maxWidth:440, maxHeight:'70vh', display:'flex', flexDirection:'column', overflow:'hidden', boxShadow:'0 20px 60px rgba(0,0,0,.15)' }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 20px', borderBottom:'1px solid #DDE4EE' }}>
              ...
              <div style={{ padding:'12px 16px 8px' }}>
                <div style={{ position:'relative' }}>
                  ...
                  <input autoFocus value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher un élève..."
                    style={{ width:'100%', paddingLeft:36, paddingRight:12, paddingTop:8, paddingBottom:8, fontSize:14, border:'1px solid #DDE4EE', borderRadius:10, outline:'none', fontFamily:'Inter,sans-serif' }} />
                </div>
              </div>
                ...
                {searchRes.length === 0
                  ...
                  : searchRes.slice(0,20).map(e => (
                    <button key={e.id} onClick={() => { setDiagModal(false); navigate(`/diagnostic-eleve?id=${e.id}`); }}
                      style={{ width:'100%', display:'flex', alignItems:'center', gap:12, padding:'10px 12px', borderRadius:10, background:'transparent', border:'none', cursor:'pointer', textAlign:'left', transition:'background .12s' }}
                      onMouseEnter={ev => ev.currentTarget.style.background = '#F1F4F8'}
                      onMouseLeave={ev => ev.currentTarget.style.background = 'transparent'}>
                      <div style={{ width:36, height:36, borderRadius:'50%', background:'#4A90C4', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, color:'#fff', flexShrink:0 }}>
                        {e.prenom?.[0]}{e.nom?.[0]}
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
