...
Deno.serve(async (req) => {
  try {
    ...
    if (!email || !profession) {
      return Response.json({ error: 'Missing email or profession' }, { status: 400 });
    }

    const registrationUrl = `${req.headers.get('origin')}/register`;
...
Pour créer votre compte, cliquez sur le lien ci-dessous et renseignez :
· Votre prénom et nom
· Votre rôle :
